...
df3 = df_avg.groupBy("patient_id").agg(F.avg("days_gap").alias("avg_days_gap"))

#4️⃣ Find the Highest Pulse Reading AND the Date It Occurred
w1 = Window.partitionBy("patient_id").orderBy(F.col("pulse").desc())
#max_pulse = df.withColumn("max_pulse", F.max("pulse"))
df4 = df.withColumn("rn", F.row_number().over(w1)) \
 .filter("rn=1") \
 .drop("rn").select("patient_id", "reading_date", "pulse")


#5️⃣ Label Overall Patient Risk Level
w2 = Window.partitionBy("patient_id").orderBy(F.col("reading_date").desc())

find_recent = ((df.withColumn("rn", F.row_number().over(w2))
               .filter("rn= 1").withColumn("risk_level", F.when(F.col("temperature") > 100.4, "High Risk")
               .when(F.col("oxygen") < 95, "Medium Risk").otherwise("Normal"))).orderBy(F.col("reading_date").desc())).drop("rn").select("patient_id", "reading_date", "risk_level")
find_recent.show()
